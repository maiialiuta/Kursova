...
class MetricsCalculator:
    ...
    # The method will be called only once time when we want to print the results
    @staticmethod
    ...
    # Calculate WMPC2 metric, by adding the number of the methods in the class + num of the parameters for each
    # method in the class The wmpc2 metrics based on that a method with more parameters in more complex that other
    # with parameters
    def calc_wmpc2(self):
        num_of_params = 0
    ...
    def calc_size2(self):
        ...
        self.class_obj.get_size_category_metrics().set_size2(len(self.class_obj.get_methods()) +
                                                             len(self.class_obj.get_fields()))
    ...
    # Calculate LCOM metric, by subtracting cohesive from non-cohesive pairs
    def calc_lcom(self):
        cohesive = 0
        non_cohesive = 0

        # A dictionary where keeps the method name and value is a set with all the class and instance attributes
        # that the method use
    ...
    def calc_rfc(self):
        ...
        remote_methods_sum = len(set(MethodsCalledNodeVisitor(self.class_obj)
                                     .visit_ClassDef(self.class_obj.get_class_ast_node())))
        self.class_obj.get_complexity_category_metrics().set_rfc(self.class_obj.get_size_category_metrics().get_nom()
                                                                 + remote_methods_sum)
    ...
    def calc_dit(self, a_parent_class_object):
        ...
        if HierarchyNodeVisitor(self.curr_dit_class).visit_ClassDef(self.curr_dit_class.get_class_ast_node()) == []:
            val = 0 if len(self.return_children(self.curr_dit_class)) == 0 else 1
            self.curr_dit_class.get_complexity_category_metrics().set_dit(val)
            # set_hierarchy is needed for the calculation of potential children classes' DIT since their DIT is ->
            # MaxParentDIT + 1
            self.curr_dit_class.set_hierarchy(val)
        # Class is part of an inheritance hierarchy (not the root)
        else:
            my_parent_classes_names_only = HierarchyNodeVisitor(self.curr_dit_class).visit_ClassDef(
                self.curr_dit_class.get_class_ast_node())

            # Converting simple name pointers to actual object classes
            my_parent_classes_objects = self.convert_to_actual_parent_objects(self.curr_dit_class,
                                                                              my_parent_classes_names_only)
            # Finding the max depth of the parent classes since DIT takes into consideration the max
            # parent class depth as input
    ...
    # Classes in libraries not included in the calculation
    def calc_cbo(self):
    ...
    def calc_wmpc1(self):
        ...
        if class_nom > 0:
            ...
            # If there are no methods, wmpc1 is equal to 0
